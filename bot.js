...
bot.start((ctx) => {
    ...
    if (chatType === 'private') {
        // Личные сообщения — тут WebApp разрешен и работает на 100%
        ctx.reply(
            "💎 **ПЛОВЦЕНТР HUB | by Денис Максатбеков))** 💎\n\n" +
        ...
        // Внутри группы — заменяем WebApp на бронебойный URL-переходник
        ctx.reply(
            "🔥 **ПЛОВЦЕНТР В ЗДАНИИ!** 🔥\n\n" +
            "Автор: Денис Максатбеков)) | Личка: @prodelal\n\n" +
...
bot.command('help', (ctx) => {
    ctx.reply(
        ...
        "Создатель: Денис Максатбеков))\n" +
        "Связь / Личка: @prodelal"
    );
});
...
bot.launch().then(() => {
    ...
    console.log('💎 Автор: Денис Максатбеков)) | Личка: @prodelal');
    console.log('=========================================');
});
